...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!authResponse.ok) {
        ...
        throw new Error(`Failed to get access token: ${authResponse.status} ${errorBody}`);
      }
    ...
    try {
      ...
      if (!verifyResponse.ok) {
        ...
        throw new Error(`Failed to verify transaction: ${verifyResponse.status} ${errorBody}`);
      }
    ...
    res.status(500).json({ verified: false, error: 'Failed to verify transaction' });
  }
}